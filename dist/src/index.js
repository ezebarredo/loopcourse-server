...
app.post("/api/:subLevelId/cards", async (request, response) => {
    ...
    const { params: { subLevelId }, body, } = request;
...
app.post("/api/:questionId/answers", async (request, response) => {
    const { params: { questionId }, body, } = request;
...
app.get("/api/levels/:levelId/sublevels/:sublevelId", async (request, response) => {
    const { params: { levelId, sublevelId }, } = request;
...
// });
// PATCH answerId
app.patch("/api/questions/:questionid/answers/:answerId", async (request, response) => {
    const { params: { questionid, answerId }, body, } = request;
...
app.patch("/api/levels/:levelId", async (request, response) => {
    const { params: { levelId }, body, } = request;
...
app.patch("/api/levels/:levelId/sublevels/:sublevelId", async (request, response) => {
    const { params: { levelId, sublevelId }, body, } = request;
    ...
    const [subLevelInfo, answer1, answer2, answer3, card1, card2, card3, card4, card5, card6, card7, card8, card9, card10,] = await prisma.$transaction([
        updateSublevelInfo,
        ...updateAnswers,
        ...updateCards,
    ]);
...
app.get("/api/sublevels/:sublevelId/cards", async (request, response) => {
    const { params: { sublevelId }, } = request;
